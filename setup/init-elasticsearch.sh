...
download_and_process_compounds() {
    ...
    local temp_dir="/tmp/compounds_download"
    ...
    for chunk in chunk_*; do
        ...
        response=$(curl -s -w "%{http_code}" -X POST "http://elasticsearch:9200/dsfp-compounds-1/_bulk" \
...
if curl -s -f "http://elasticsearch:9200/dsfp-screening-index" > /dev/null; then
    echo "Index 'dsfp-screening-index' already exists"
else
    echo "Creating index 'dsfp-screening-index'..."
    curl -X PUT "http://elasticsearch:9200/dsfp-screening-index" -H "Content-Type: application/json" -d @"/mappings.json"
fi

if curl -s -f "http://elasticsearch:9200/dsfp-compounds-1" > /dev/null; then
    echo "Index 'dsfp-compounds-1' already exists"
else
...
# Note: Tracking index creation removed - now using DuckDB for tracking
echo "Tracking functionality migrated to DuckDB - no ES tracking index needed"

current_count=$(curl -s "http://elasticsearch:9200/dsfp-compounds-1/_count" | grep -o '"count":[0-9]*' | cut -d':' -f2)
echo "Current document count: $current_count"

if [ "$current_count" = "0" ]; then
    echo "Index is empty, downloading and loading compounds..."
    if download_and_process_compounds; then
        echo "✓ Compounds loaded successfully"
    else
        echo "✗ Failed to load compounds"
    fi
else
    echo "Index already has $current_count documents, skipping upload"
fi
